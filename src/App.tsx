...
type Operation = [string, Operator | null, string];

const getValue = (value: string) => Number(value.replace(",", "."));
const valueToString = (value: number) => String(value).replace(".", ",");
...
const explodeOperation = (operation: string): Operation => {
  const result = /(.*)([-x/+])(.*)/.exec(operation.replace(/,/g, "."));
  if (!result) return ["", null, ""];
  const [_, left, operator, right] = result;
  return [left, operator, right] as Operation;
};
...
function App() {
  ...
  const setResult = React.useCallback(() => {
    const result = valueToString(getResult(explodeOperation(display)) || 0);
    setDisplay(result);
    setNumber1(result);
    setNumber2("");
    setOperator("");
  ...
  const handleToken = React.useCallback(
    ...
    [display, operator, setResult, decimal1, decimal2, number1, number2]
  );
